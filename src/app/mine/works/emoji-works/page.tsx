...
import { FolderOpen, Loader2, ChevronRight, Layers3 } from "lucide-react";
...
export default function EmojiWorksPage() {
  ...
  const loadCollections = async (nextPage: number, append: boolean) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErrorMessage(msg || "加载表情包作品失败，请稍后重试");
        if (!append) {
          setItems([]);
          setTotal(0);
        }
      ...
      setErrorMessage("加载表情包作品失败，请稍后重试");
      if (!append) {
        setItems([]);
        setTotal(0);
      }
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900">表情包作品</h1>
          <p className="mt-1 text-sm text-slate-500">展示你上传的表情包合集，点击查看详情</p>
        </div>
      ...
      {!loading && items.length === 0 ? (
        <div className="rounded-[2.25rem] border-2 border-dashed border-slate-200 bg-white py-16 text-center">
          ...
          <h3 className="text-lg font-black text-slate-900">还没有表情包作品</h3>
          <p className="mt-2 text-sm text-slate-500">请先到“我的 - 我的上传”创建合集并上传素材</p>
          <Link
          ...
          return (
            ...
            >
              ...
              <div className="space-y-3 p-4">
                ...
                <Link
                  href={`/mine/works/emoji-works/${item.id}`}
